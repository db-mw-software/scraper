...
const sep = '\t';
const showsFileName = './allconcerts.tsv';
const artistAtShowFileName = './artistatconcert.tsv';
const artistsFileName = './allartists.tsv';
const venues = [
...
fs.writeFile(artistAtShowFileName, '[', (error) => {
  if (error) console.log(`Error writing blank ${artistAtShowFileName}`, error);
  else
...
function parseVenue(events, venueName) {
  ...
  for (let i = 0; i < eventlist.length; i++) {
    ...
    if (event.type === 'Concert') {
      // concertID, title, datetime, venueID, venueName, ageRestriction
      shows += concertRowFor(event, sep) + '\n';
      //ArtistAtConcert: band1ID, band2iD, band3ID, band4ID, etc.
      artistAtShows += event.performance;
    }
  }
